...
'@
    $compatibleRequirements | Out-File -FilePath $tempRequirements -Encoding utf8
...
Write-Host "  4. Run your commands (python main.py --mode backtest --config config/config_optimized.yaml)" -ForegroundColor White
...
try {
    $testResult = & (Join-Path $venvPath "Scripts\python.exe") -c "import sys, numpy, pandas, yaml; print(f'✓ Python: {sys.version}'); print(f'✓ NumPy: {numpy.__version__}'); print(f'✓ Pandas: {pandas.__version__}'); print(f'✓ YAML: {yaml.__version__}'); print('✓ All core dependencies loaded successfully!')"
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "`n✅ Virtual environment test passed!" -ForegroundColor Green
        Write-Host $testResult -ForegroundColor White
    }
    else {
        Write-Host "`n❌ Virtual environment test failed!" -ForegroundColor Red
    }
}
catch {
    Write-Host "`n❌ Virtual environment test failed: $_" -ForegroundColor Red
}
